...
When the user asks to create a release, follow this exact workflow. A release
consists of **two steps**: a git tag AND a GitHub Release. A tag alone is
NOT a release — it won't appear on the Releases page and the OctoPrint plugin
manager won't see it.

### Step 1: Determine the Next Version

Run `git tag --sort=-v:refname` to inspect the latest tags. The convention is:
...
The current series is `1.x`. Tags follow the pattern:
- `1.1` / `1.2` / `1.3` — minor releases (feature drops)
- `1.1.1` / `1.1.2` / `1.2.1` — patch releases (bug fixes)

When in doubt, default to a PATCH bump (e.g., `1.1.1` after `1.1`).

### Step 2: Commit and Tag

```powershell
git add <files>
git commit -m "fix: <short description>" -m "- <bullet 1>" -m "- <bullet 2>"
...
**CRITICAL** — a `git tag` push does NOT create a GitHub Release. You MUST run:
...
Always verify the release was created:
```powershell
gh release view <VERSION> --repo FracktalWorks/ControlCenter
```

The release URL will be `https://github.com/FracktalWorks/ControlCenter/releases/tag/<VERSION>`.

### Step 4: Verify the Source Archive

The OctoPrint plugin manager installs from this URL (auto-generated by GitHub):
```
https://github.com/FracktalWorks/ControlCenter/archive/refs/tags/<VERSION>.zip
```
This archive is auto-created when the tag is pushed — no extra step needed.

### Release Notes Format

Follow this template (see `1.1.1` for reference):
```markdown
## <Category>

- **<Bold highlight>**: <Plain description>.
- **<Bold highlight>**: <Plain description>.

**Full Changelog**: https://github.com/FracktalWorks/ControlCenter/compare/<OLD>...<NEW>
```

Keep release notes concise. Use bullet points with bold highlights. Always include the Full Changelog comparison link.
